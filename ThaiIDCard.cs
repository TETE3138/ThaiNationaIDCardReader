...
namespace ThaiNationalIDCard
{
    ...
    public class ThaiIDCard : IDisposable
    {
        ...
        private void _Inserted(CardStatusEventArgs args)
        {
            ...
            catch (PCSCException ex)
            {
                ...
                Debug.WriteLine(_error_message);
            }
        ...
        {
            if (eventPhotoProgress != null)
                foreach (Delegate d in eventPhotoProgress.GetInvocationList())
                {
                    eventPhotoProgress -= (handlePhotoProgress)d;
                }
            if (eventCardInserted != null)
                foreach (Delegate d in eventCardInserted.GetInvocationList())
                {
                    eventCardInserted -= (handleCardInserted)d;
                }
            if (eventCardInsertedWithPhoto != null)
                foreach (Delegate d in eventCardInsertedWithPhoto.GetInvocationList())
                {
                    eventCardInsertedWithPhoto -= (handleCardInserted)d;
                }
            if (eventCardRemoved != null)
                foreach (Delegate d in eventCardRemoved.GetInvocationList())
                {
                    eventCardRemoved -= (handleCardRemoved)d;
                }
        }
    } // End class
